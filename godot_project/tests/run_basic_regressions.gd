...
func _init() -> void:
	...
	if FileAccess.file_exists(import_manifest_abs):
		...
		if import_manifest is Dictionary:
			_assert_eq(int(import_manifest.get("format_version", -1)), 4, "Manifest format_version")
	...
	_assert_prefab_report_entry(semantic_prefabs, str(sample_prefabs.get("polygon_body", "")), "approximated", "polygon_collider_imported", true)
	...
	if compatibility_report is Dictionary:
		...
		_assert_eq(str(polygon_body_entry.get("tier", "")), "approximated", "Compatibility report polygon-body tier")
		...
		_assert_true(Array(polygon_body_entry.get("reasons", [])).has("rigidbody_deferred"), "Compatibility report polygon-body rigidbody reason")
